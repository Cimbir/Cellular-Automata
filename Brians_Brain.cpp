// g++ -c .\Brians_Brain.cpp -ID:\Workspace\SFML-2.6.1\include
// g++ .\Brians_Brain.o -o BB -LD:\Workspace\SFML-2.6.1\lib -lsfml-graphics -lsfml-window -lsfml-system
...
int getStateByRules(cord from, vector<vector<int>>& grid){
    ...
    if(curState > 0) return curState-1;
        ...
        int liveAm = 0;
        for(int i = 0; i < neighbors.size(); i++) liveAm += (grid[neighbors[i].first][neighbors[i].second] == 2); 

        if(liveAm == 2) return 2;
        return 0;
    }
}
...
RectangleShape stateToRect(int state){
    ...
    if(state == 1) rect.setFillColor(Color::White);
    if(state == 2) rect.setFillColor(Color::Red);
    return rect;
}
