// g++ -c .\Moore_Neighborhood.cpp -ID:\Workspace\SFML-2.6.1\include
// g++ .\Moore_Neighborhood.o -o MN -LD:\Workspace\SFML-2.6.1\lib -lsfml-graphics -lsfml-window -lsfml-system
...
int getStateByRules(cord from, vector<vector<int>>& grid){
    ...
    for(int i = 0; i < neighbors.size(); i++){
        cord next = neighbors[i];
        if(abs(next.first-from.first) > 1 || abs(next.second-from.second) > 1 || grid[next.first][next.second] == 0) wallAm++;
    }
...
void setUpGridRandom(vector<vector<int>>& grid){
    srand(time(0));
    int range = NOISE_DENSITY*NOISE_DENSITY;
    int threshold = NOISE_DENSITY*sqrt(NOISE_DENSITY);
    for(int i = 0; i < N; i++){
        for(int j = 0; j < M; j++){
            // Depends on rule variant
            grid[i][j] = (rand()%range < threshold);
        }
    }
}
